...
def remote_template(template):
    # URL of the remote template
    remote_template_url = os.getenv('REMOTE_TEMPLATE_URL')
    # Fetch the template content
    response = requests.get(f'{remote_template_url}/{template}')
...
def register_routes(app):
    ...
    @app.route('/')
    def home():
        #temp = remote_template('index.html')
        ...
        if form.validate_on_submit():
            ...
            if not user:
                ...
                if not form.picture.data:
                    ...
                    form.picture.data.save(os.path.join(os.getenv('PROFILE_PICS_URL'), filename))  # Save the image
